...
> CNN由多个layers组成。 每个layer有非常直接的功能： transform an input 3D volumn to an output 3D volume, with some differentiable function (have/no parameters)。
...
> CNN中有三种典型的layers: CONV layer, POOLING Layer, FC layer.
...
> CIFAR-10中的网络结构 `[INPUT - CONV - RELU - POOL - FC]`

> - INPUT [32x32x3] 存储images的raw pixel values, 3代表RGB三个通道

> - CONV layer: 根据input的local region的连接，计算相应neurons的输出。 每个计算 neuron的weights与neuron连接的input region的dot product。 这个过程会改变volume的size，比如[32x32x12].

> - RELU layer：使用 elmentwise activation function对activation进行处理， 这个不改变volumn的大小.

> - POOL layer: 对spatial dimensions (width/height进行downsampling处理)，并改变volume的大小，如[16x16x12].

> - FC layer: 计算class scores, 最后的volume大小为[1x1x12], 10代表了class score, 比如CIFAR-10中的10个categories. FC layer中的每个neuron都会与previous volume的所有进行连接。    

In this way, ConvNets transform the original image layer by layer from the original pixel values to the final class scores. Note that some layers contain parameters and other don't. In particular, the CONV/FC layers perform transformations that are a function of not only the activations in the input volume, but also of the parameters (the weights and biases of the neurons). On the other hand, the RELU/POOL layers will implement a fixed function. The parameters in the CONV/FC layers will be trained with gradient descent so that the class scores that the ConvNet computes are consistent with the labels in the training set for each image. 

> CNN 通过层与层将original image的pixel value转换为final class scores. 其中某些layers包含模型参数，但某些层则不包含。 比如 CONV/FC layers的function对input volume的activations和neuron的weights进行转换， 而 RELU/POOL layers则是fixed function.

> CONV/FC layers通过梯度下降进行训练， 从而使得CNN计算的训练集图片的class scores与labels保持一致。
...
> `上图中结论`: 图中的每一层的activation value显示为图片， 代表了每一层的neuron针对input image pixel所具有的选择特性。
...
> `LeNet`: 由Yann LeCun在90年代设计， 这个结构用于读取zip codes, digits的应用。
...
> `AlexNet`: 该net取得了2012年ILSVR比赛的冠军， 超出第二名16%度的性能。 这个net和LeNet很相似，但是更加deeper, bigger， 而且使用了stacked CNN layers (以前更常见的是单个cnn layer+pool layer的结构。)
...
> `ZF Net`: ILSVRC 2013的冠军。 对AlexNet进行了改进， 特别是对中间的cnn layers进行了扩展，使得层次数量更多。
...
> `GoogLeNet`: ILSVRC 2014的冠军。 主要是提出了一种 Inception Moudule, 使得模型参数显著减少(从60M减少到4M)。
...
> `VGGNet`: ILSVRC 2014的runner-up. 其主要贡献是 验证了网络的深度对获取更高的性能有重要影响。 该网络最好的结果包含了16 CONV/FC layers， 而且是homogeneous architecture，从网络开始到结束只进行了3x3卷积和2x2池化。 尽管其性能比GoogLeNet稍差， 但将其用于其他tasks的transfer learning时效果比GoogLeNet要好。 因此， VGG net成为了当前最推荐的网络，用于提取CNN features。 其缺点是， 模型参数多(140M), 内存消耗大。
...
> 对于VGG Net， 其大部分memory在于early CONV layers, 大部分parameters在于last FC layers. 在此特殊情况下， first FC layer包含100M weights(总共有140M weights)。
...
> 构建CNN网络的最大瓶颈在于 memory消耗。 现代GPU板子一般内存在3/4/6GB, 最好的有12GB。 实际中有三个策略用于考虑memory消耗：

> 1. intermediate volume sizes: 这些代表CNN网络每层的原始activations，以及他们gradients的数量。 通常，大部分activations存在于CNN网络的前面层， 他们需要保留(因为在backpropagation中会使用到他们)。 一个更有效的实现方法是， 在测试时只存储当前的activations at any layer，而不需要之前的activations on layers below.

> 2. parameter size: 这些代表CNN网络的参数的数量， 以及其在backpropagation中的梯度。 因此， 用于存储parameter vector的memory通常需要3倍以上的考虑。

> 3. miscellaneous: 输入数据，如image data, their augmented versions.
